...
class GreyFieldDialog extends React.Component {
    ...
    renderTableHeader() {
        ...
        return this.state.currentCategoryCombo && (
            this.state.categoryCombos.get(this.state.currentCategoryCombo).categories.toArray().map((cat, catNum) => {
                ...
                const isLastHeader = catNum ===
                    this.state.categoryCombos.get(this.state.currentCategoryCombo).categories.size - 1;
                const row = (
                    <tr key={catNum}>
                    ...
                    return (
                        <tr key={deNum} style={{ background: deNum % 2 === 0 ? 'none' : '#f0f0f0' }}>
                            ...
                            {cocFields.map((fields, fieldNum) =>
                                this.renderCheckbox(dse.dataElement, fields, fieldNum))}
                        </tr>
                    );
                }) : null;
    }
    ...
    render() {
        ...
        const uniqueCatComboIds = [];
        let sectionDataElementIds = [];
        let categoryCombosForSection = [];
